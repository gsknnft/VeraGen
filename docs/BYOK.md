...
BYOK covers generation billing only. Hosting, storage and MP4 rendering remain application infrastructure costs. Existing project and collection routes still need authenticated ownership checks before a public multi-user deployment; a provider connection is not a VeraGen login.
